#!/bin/bash
# =============================================================================
# api0-landing VPS Setup Script
# =============================================================================
# Run this script on your VPS as root:
#   sudo bash setup-api0-vps.sh
# 
...
# --- Configuration ---
DOMAIN="api0.ai"
DOMAIN_WWW="www.api0.ai"
REPO="git@github.com:bennekrouf/landing-api0.git"
APP_DIR="/var/www/api0-landing"
APP_PORT=3005
...
# --- Pre-flight checks ---
if [ "$EUID" -ne 0 ]; then
  echo -e "\033[0;31m[X]\033[0m Please run as root: sudo bash setup-api0-vps.sh"
  exit 1
fi
...
if [ ! -f "/var/www/.ssh/id_ed25519" ]; then
  mkdir -p /var/www/.ssh
  ssh-keygen -t ed25519 -C 'api0-deploy' -f /var/www/.ssh/id_ed25519 -N ''
...
echo ""
echo -e "${YELLOW}================================================================${NC}"
echo -e "${YELLOW}  ENSURE THIS DEPLOY KEY IS ADDED TO GITHUB LANDING-API0      ${NC}"
echo -e "${YELLOW}                                                                ${NC}"
echo -e "${YELLOW}  Repository: https://github.com/bennekrouf/landing-api0      ${NC}"
...
echo -e "${YELLOW}================================================================${NC}"
echo ""
read -p "Press Enter once the deploy key is added to landing-api0..." _
...
sudo -u "$APP_USER" HOME=/var/www bash -c "
  ...
  $(which pm2) save
"
log "api0-landing built and running via PM2 on port $APP_PORT"

# =============================================================================
step "4/4 — Configure Nginx & SSL"
# =============================================================================
cat > /etc/nginx/sites-available/api0 <<NGINX
server {
    listen 80;
    server_name $DOMAIN $DOMAIN_WWW;

    access_log /var/log/nginx/api0_access.log;
    error_log /var/log/nginx/api0_error.log;
...
ln -sf /etc/nginx/sites-available/api0 /etc/nginx/sites-enabled/api0
...
# =============================================================================
step "api0-landing Deployment Complete!"
# =============================================================================
